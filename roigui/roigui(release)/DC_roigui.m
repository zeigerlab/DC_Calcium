function DC_roigui(im,savename,autoROIsave,ROI_list,pixel_dimension)
%   
...
%   Adapted 09/19/2014        by Daniel Cantu

% Notes: It should be possible to automatically export the picture of the ROI
% traces with numbers. Additionally, it should be possible to reopen saved data.
% I have spent several hours attempting both of these, but have yet to succeed.
% They'd be useful things to have for reference, so perhaps in a future version. DC 9/20/14
...
seed_location.col=[];
ROIcount=1;
if nargin<4 %Changed 9/21/14 DC because of the added input arguments.
    ROI_list=[];
else
    if ~isempty(ROI_list)
...
d.name='Standard Deviation'; %Added 9/19/14 DC
d.data=std(im,0,3);
d.playsize=1;
d.DataRange=[min(d.data(:)),max(d.data(:))];
% d.DataRange=dataRange;
...
d.name='Median Abs. Dev.'; %Added 9/19/14 DC. Median Absolute Deviation as an option for viewing.
d.data=mad(im,1,3);
d.playsize=1;
d.DataRange=[min(d.data(:)),max(d.data(:))];
% d.DataRange=dataRange;
...
uicontrol(hctrlfig,'Tag','BnExport','Style','pushbutton','Units','pixel','position',[260,160,70,25],'String','Save!','FontWeight','bold','Callback',@BnExportCallback,'BackgroundColor',[0 1 1]);
...
%         xlim([0,max(timebase)]);
    ...
    function BnExportCallback(varargin)
        ...
        %Wait, is this seriously it? Yeah, I'm changing this. DC 09/20/14 Everything below to make this a proper save function.
        save(savename,'ROI_list'); %saves a file with the ending "_data.mat" where from whence it was loaded.
        %stick ROI num onto picture: This is much harder than it should be. Skipping for now. DC 09/20/14
    end
...
%                 ROI=measure_ROI(im,ROI);
    ...
    function CreateNewDispWnd() 
        ...
        axes('Position',[0 0 1 1],'Layer','top','DataAspectRatio',[1 1 1]); %Added to maximize display while retaining aspect ratio DC 09/21/14
    ...
    function drawROI()
        ...
        if autoROIsave==1
        save(savename,'ROI_list'); %Boom, autosaved. Added 9/21/14 DC. This might slow things down. I'll make it optional.
        end
