function varargout = DC_roi_detect_old(varargin)
% DC_ROI_DETECT_OLD MATLAB code for DC_roi_detect_old.fig
%      DC_ROI_DETECT_OLD, by itself, creates a new DC_ROI_DETECT_OLD or raises the existing
%      singleton*.
%
%      H = DC_ROI_DETECT_OLD returns the handle to a new DC_ROI_DETECT_OLD or the handle to
%      the existing singleton*.
%
%      DC_ROI_DETECT_OLD('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in DC_ROI_DETECT_OLD.M with the given input arguments.
%
%      DC_ROI_DETECT_OLD('Property','Value',...) creates a new DC_ROI_DETECT_OLD or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before DC_roi_detect_old_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to DC_roi_detect_old_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help DC_roi_detect_old

% Last Modified by GUIDE v2.5 01-Feb-2018 10:41:12

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @DC_roi_detect_old_OpeningFcn, ...
                   'gui_OutputFcn',  @DC_roi_detect_old_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before DC_roi_detect_old is made visible.
function DC_roi_detect_old_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to DC_roi_detect_old (see VARARGIN)

% Choose default command line output for DC_roi_detect_old
...
% UIWAIT makes DC_roi_detect_old wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = DC_roi_detect_old_OutputFcn(hObject, eventdata, handles) 
...
for i=1:file_num
    ...
    [progress]=DC_roi_detect_process(filename,autoroi,handles,progress); 
