function varargout = DC_roi_autoselect(varargin)
% DC_ROI_AUTOSELECT MATLAB code for DC_roi_autoselect.fig
%      DC_ROI_AUTOSELECT, by itself, creates a new DC_ROI_AUTOSELECT or raises the existing
%      singleton*.
%
%      H = DC_ROI_AUTOSELECT returns the handle to a new DC_ROI_AUTOSELECT or the handle to
%      the existing singleton*.
%
%      DC_ROI_AUTOSELECT('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in DC_ROI_AUTOSELECT.M with the given input arguments.
%
%      DC_ROI_AUTOSELECT('Property','Value',...) creates a new DC_ROI_AUTOSELECT or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before DC_roi_autoselect_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to DC_roi_autoselect_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help DC_roi_autoselect

% Last Modified by GUIDE v2.5 07-Sep-2016 12:33:01

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @DC_roi_autoselect_OpeningFcn, ...
                   'gui_OutputFcn',  @DC_roi_autoselect_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before DC_roi_autoselect is made visible.
function DC_roi_autoselect_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to DC_roi_autoselect (see VARARGIN)

% Choose default command line output for DC_roi_autoselect
...
if exist('autosave_DC_autoroi.mat','file')==2 %Checks for autosave file
    load('autosave_DC_autoroi.mat'); %loads file into workspace
    write_autoroi(handles,autoroi,2) %Load settings into GUI
else
    DC_autoload_fail %Runs dialog box to find and move an autoload file
    if exist('autosave_DC_autoroi.mat','file')==2 %If no autoload selected, create default
        load('autosave_DC_autoroi.mat'); %loads file into workspace
...
% UIWAIT makes DC_roi_autoselect wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = DC_roi_autoselect_OutputFcn(hObject, eventdata, handles) 
...
%Autosave
save('autosave_DC_autoroi.mat','autoroi');
...
for i=1:file_num
    ...
    [progress]=DC_roi_autoselect_process(filename,autoroi,handles,progress);
