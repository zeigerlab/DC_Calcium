function varargout = DC_noise_reduction(varargin)
% DC_NOISE_REDUCTION MATLAB code for DC_noise_reduction.fig
%      DC_NOISE_REDUCTION, by itself, creates a new DC_NOISE_REDUCTION or raises the existing
%      singleton*.
%
%      H = DC_NOISE_REDUCTION returns the handle to a new DC_NOISE_REDUCTION or the handle to
%      the existing singleton*.
%
%      DC_NOISE_REDUCTION('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in DC_NOISE_REDUCTION.M with the given input arguments.
%
%      DC_NOISE_REDUCTION('Property','Value',...) creates a new DC_NOISE_REDUCTION or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before DC_noise_reduction_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to DC_noise_reduction_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help DC_noise_reduction

% Last Modified by GUIDE v2.5 16-Apr-2015 13:26:48

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @DC_noise_reduction_OpeningFcn, ...
                   'gui_OutputFcn',  @DC_noise_reduction_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before DC_noise_reduction is made visible.
function DC_noise_reduction_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to DC_noise_reduction (see VARARGIN)

% Choose default command line output for DC_noise_reduction
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes DC_noise_reduction wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = DC_noise_reduction_OutputFcn(hObject, eventdata, handles) 
