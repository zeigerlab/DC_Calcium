function varargout = DC_motion_correction(varargin)
% DC_MOTION_CORRECTION MATLAB code for DC_motion_correction.fig
%      DC_MOTION_CORRECTION, by itself, creates a new DC_MOTION_CORRECTION or raises the existing
%      singleton*.
%
%      H = DC_MOTION_CORRECTION returns the handle to a new DC_MOTION_CORRECTION or the handle to
%      the existing singleton*.
%
%      DC_MOTION_CORRECTION('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in DC_MOTION_CORRECTION.M with the given input arguments.
%
%      DC_MOTION_CORRECTION('Property','Value',...) creates a new DC_MOTION_CORRECTION or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before DC_motion_correction_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to DC_motion_correction_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help DC_motion_correction

% Last Modified by GUIDE v2.5 24-Oct-2018 14:25:03

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @DC_motion_correction_OpeningFcn, ...
    'gui_OutputFcn',  @DC_motion_correction_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before DC_motion_correction is made visible.
function DC_motion_correction_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to DC_motion_correction (see VARARGIN)

%Compiler code: mcc -mv -R -singleCompThread DC_motion_correction.m
...
autosave_file='autosave_DC_motcor.mat'; %Name autosave file

%Check if autoload exists
if exist('autosave_DC_motcor.mat','file')==2 %Checks for autosave file
    load('autosave_DC_motcor.mat'); %loads file into workspace
    ...
    if exist('autosave_DC_motcor.mat','file')==2 %If no autoload selected, create default
        load('autosave_DC_motcor.mat'); %loads file into workspace
...
% UIWAIT makes DC_motion_correction wait for user response (see UIRESUME)
% uiwait(handles.figure1);

% --- Outputs from this function are returned to the command line.
function varargout = DC_motion_correction_OutputFcn(hObject, eventdata, handles)
...
%Autosave
save('autosave_DC_motcor.mat','motcor');
...
for i=1:file_num
    ...
    [progress,motcor]=DC_motion_correction_process(filename,motcor,handles,progress);
