...
for imagedex = 1:num_frames %For every frame of the video
    ...
    if imagedex==1 %for the first picture, overwrite. Then, append.
        imwrite(secondIdscale(:,:,imagedex),[fullvidfile2(1:end-4) '_DCmcor2_ch2' '.tif'],'tiff','Compression','none','WriteMode','overwrite');
        imwrite(secondIGdscale(:,:,imagedex),[fullvidfile1(1:end-4) '_DCmcor2_ch1' '.tif'],'tiff','Compression','none','WriteMode','overwrite');
    else %appends the rest of the images to the file in order
        imwrite(secondIdscale(:,:,imagedex),[fullvidfile2(1:end-4) '_DCmcor2_ch2' '.tif'],'tiff','Compression','none','WriteMode','append');
        imwrite(secondIGdscale(:,:,imagedex),[fullvidfile1(1:end-4) '_DCmcor2_ch1' '.tif'],'tiff','Compression','none','WriteMode','append');
    end
end
