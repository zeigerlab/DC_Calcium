function DC_motion_correct_1channel_1step(fullvidfile,blocksize)
...
if BG_sub==1
    ...
    imwrite(greenchannel(:,:,1)/scalefactor,[fullvidfile(1:end-4) '_DC_BGsub' '.tif'],'tiff','Compression','none','WriteMode','overwrite');
    for imagedex = 2:num_frames %start at frame two and append to first image
        ...
        imwrite(greenchannel(:,:,imagedex)/scalefactor,[fullvidfile(1:end-4) '_DC_BGsub' '.tif'],'tiff','Compression','none','WriteMode','append');
    end
end
...
%Save video file
display('Saving video...')
imwrite(firstIdscale(:,:,1),[fullvidfile(1:end-4) '_DCmcor1' '.tif'],'tiff','Compression','none','WriteMode','overwrite');
for imagedex = 2:num_frames %start at frame two and append to first image
    if floor(imagedex/10)==(imagedex/10) %show only every 10 frames progress
        display(['Progress: ' num2str((imagedex)/(num_frames)*100) '%']);
    end
    imwrite(firstIdscale(:,:,imagedex),[fullvidfile(1:end-4) '_DCmcor1' '.tif'],'tiff','Compression','none','WriteMode','append');
end
