function [spikes,hz]= DC_F_to_spikes(raw_data,data_type,name)
...
hz=DC_spikes_to_hz(spikes,frames_per_bin,frame_duration);

%Spiking graph
spike_filename=[name '_spikes.pdf'];
DC_multiplot(spikes,10,spike_filename,frame_duration,'(sec)',8,'dF/F',2);

%Firing rate graph. Not really different from spiking if you don't bin.
hz_filename=[name '_Hz.pdf'];
DC_multiplot(hz,10,hz_filename,frame_duration,'(sec)',8,'Hz',2);
