function varargout = DC_roi_detect(varargin)
% DC_ROI_DETECT MATLAB code for DC_roi_detect.fig
%      DC_ROI_DETECT, by itself, creates a new DC_ROI_DETECT or raises the existing
%      singleton*.
%
%      H = DC_ROI_DETECT returns the handle to a new DC_ROI_DETECT or the handle to
%      the existing singleton*.
%
%      DC_ROI_DETECT('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in DC_ROI_DETECT.M with the given input arguments.
%
%      DC_ROI_DETECT('Property','Value',...) creates a new DC_ROI_DETECT or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before DC_roi_detect_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to DC_roi_detect_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help DC_roi_detect

% Last Modified by GUIDE v2.5 01-Feb-2018 11:38:48

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @DC_roi_detect_OpeningFcn, ...
                   'gui_OutputFcn',  @DC_roi_detect_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before DC_roi_detect is made visible.
function DC_roi_detect_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to DC_roi_detect (see VARARGIN)

% Choose default command line output for DC_roi_detect
...
autosave_file='autosave_DC_autoroi.mat'; %Name autosave file
...
% UIWAIT makes DC_roi_detect wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = DC_roi_detect_OutputFcn(hObject, eventdata, handles) 
