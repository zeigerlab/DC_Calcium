function varargout = DC_calcium(varargin)
% DC_CALCIUM M-file for DC_calcium.fig
%      DC_CALCIUM, by itself, creates a new DC_CALCIUM or raises the existing
%      singleton*.
%
%      H = DC_CALCIUM returns the handle to a new DC_CALCIUM or the handle to
%      the existing singleton*.
%
%      DC_CALCIUM('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in DC_CALCIUM.M with the given input arguments.
%
%      DC_CALCIUM('Property','Value',...) creates a new DC_CALCIUM or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before DC_calcium_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to DC_calcium_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help DC_calcium

% Last Modified by GUIDE v2.5 21-Sep-2014 14:48:24

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @DC_calcium_OpeningFcn, ...
                   'gui_OutputFcn',  @DC_calcium_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before DC_calcium is made visible.
function DC_calcium_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to DC_calcium (see VARARGIN)

% Choose default command line output for DC_calcium
handles.output = hObject;
% Update handles structure
guidata(hObject, handles);

% UIWAIT makes DC_calcium wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = DC_calcium_OutputFcn(hObject, eventdata, handles) 
...
DC_roigui(im,savename,autoROIsave)
...
if autoloadstatus==1 %Autoloads if box checked
    ...
    if correctiontype==1
        roihandle=[fullvidfile(1:end-4) '_DCmcor1' '.tif'];
    else
        roihandle=[fullvidfile1(1:end-4) '_DCmcor2_ch1' '.tif'];
    end
...
if correctiontype==1
    ...
    %DC_motion_correct_1channel_1step(fullvidfile,referenceframe,blocksize);
    
    
    DC_motion_correct_1channel(fullvidfile,referenceframe,blocksize);
   
else
    %2 channel correction
    
    DC_motion_correct_2channel_1step_fast(fullvidfile1,fullvidfile2,referenceframe,blocksize);
