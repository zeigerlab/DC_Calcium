function DC_motion_correct_2channel_1step_fast(fullvidfile1,fullvidfile2,referenceframe,blocksize)
%varargout = DC_motion_correct_1channel(fullvidfile,referenceframe,blocksize)
...
redchannelmean=mean(redchannel,3); %just use the first frame
...
greenchannelmean=mean(greenchannel,3); %just use the first frame
...
parfor imagedex = 1:num_frames %For every frame of the video
    ...
    [redchannel(:,:,imagedex),greenchannel(:,:,imagedex),~,~]=doLucasKanade2(T,I,Nbasis,IG); %runs the motion correction code
    
    redchannel(:,:,imagedex)=redchannel(:,:,imagedex)/redscalefactor;
    greenchannel(:,:,imagedex)=greenchannel(:,:,imagedex)/scalefactor;
...
imwrite(redchannel(:,:,1),[fullvidfile2(1:end-4) '_DCmcor2_ch2' '.tif'],'tiff','Compression','none','WriteMode','overwrite');
imwrite(greenchannel(:,:,1),[fullvidfile1(1:end-4) '_DCmcor2_ch1' '.tif'],'tiff','Compression','none','WriteMode','overwrite');

for imagedex=2:num_frames 
    if floor(imagedex/10)==(imagedex/10) %show only every 10 frames progress
        display(['Progress: ' num2str((imagedex)/(num_frames)*100) '%']);
    end
    imwrite(redchannel(:,:,imagedex),[fullvidfile2(1:end-4) '_DCmcor2_ch2' '.tif'],'tiff','Compression','none','WriteMode','append');
    imwrite(greenchannel(:,:,imagedex),[fullvidfile1(1:end-4) '_DCmcor2_ch1' '.tif'],'tiff','Compression','none','WriteMode','append');
