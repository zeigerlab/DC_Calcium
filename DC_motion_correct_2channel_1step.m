function DC_motion_correct_2channel(fullvidfile1,fullvidfile2,referenceframe,blocksize)
...
parfor imagedex = 1:num_frames %For every frame of the video
    ...
    firstId(:,:,imagedex)=firstId(:,:,imagedex)/redscalefactor;
    firstIGd(:,:,imagedex)=firstIGd(:,:,imagedex)/scalefactor;
...
imwrite(firstId(:,:,1),[fullvidfile2(1:end-4) '_DCmcor2_ch2' '.tif'],'tiff','Compression','none','WriteMode','overwrite');
imwrite(firstIGd(:,:,1),[fullvidfile1(1:end-4) '_DCmcor2_ch1' '.tif'],'tiff','Compression','none','WriteMode','overwrite');

for imagedex=2:num_frames 
    if floor(imagedex/10)==(imagedex/10) %show only every 10 frames progress
        display(['Progress: ' num2str((imagedex)/(num_frames)*100) '%']);
    end
    imwrite(firstId(:,:,imagedex),[fullvidfile2(1:end-4) '_DCmcor2_ch2' '.tif'],'tiff','Compression','none','WriteMode','append');
    imwrite(firstIGd(:,:,imagedex),[fullvidfile1(1:end-4) '_DCmcor2_ch1' '.tif'],'tiff','Compression','none','WriteMode','append');
