function DC_motion_correct_1channel(fullvidfile,set_motcor)
...
firstIdscale=greenchannel/scalefactor; %nullify first pass
for imagedex = 1:num_frames %For every frame of the video
    %display(imagedex);
    if floor(imagedex/10)==(imagedex/10) %show only every 10 frames progress
    ...
    if imagedex==1 %for the first picture, overwrite. Then, append.
        imwrite(secondIdscale(:,:,imagedex),[fullvidfile(1:end-4) '_DCmcor1' '.tif'],'tiff','Compression','none','WriteMode','overwrite');
    else %appends the rest of the images to the file in order
        imwrite(secondIdscale(:,:,imagedex),[fullvidfile(1:end-4) '_DCmcor1' '.tif'],'tiff','Compression','none','WriteMode','append');
    end
end
